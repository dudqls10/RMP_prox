...
def ConnectToCB(ip):
    ...
    cmd_connect = CMDSock.connect_ex((ip, CMD_PORT)) == 0
    data_connect = DATASock.connect_ex((ip, DATA_PORT)) == 0

    if cmd_connect and data_connect:
        print('CONNECTED TO CB!!')

        DATAREQ_THREAD = threading.Thread(target=ReqDataStart, args=(DATASock,), daemon=True)
        CMDREAD_THREAD = threading.Thread(target=ReadCMD, args=(CMDSock,), daemon=True)
        DATAREAD_THREAD = threading.Thread(target=ReadDATA, args=(DATASock,), daemon=True)
...
def DisConnectToCB():
    ...
    for sock_name in ('CMDSock', 'DATASock'):
...
def GetCurrentJoint():
    if data_connect and systemstat_global.jnt_ref is not None and len(systemstat_global.jnt_ref) >= 6:
...
def GetCurrentMeasuredSplitedJoint():
    if data_connect and systemstat_global.jnt_ang is not None and len(systemstat_global.jnt_ang) >= 6:
        current_joint_ = (systemstat_global.jnt_ang[0], systemstat_global.jnt_ang[1], systemstat_global.jnt_ang[2]
                          , systemstat_global.jnt_ang[3], systemstat_global.jnt_ang[4], systemstat_global.jnt_ang[5])
        return current_joint_
    current_joint_ = ()
    return current_joint_
...
def GetCurrentTCP():
    if data_connect and systemstat_global.tcp_ref is not None and len(systemstat_global.tcp_ref) >= 6:
...
def GetCurreJP():
    if (
        data_connect and
        systemstat_global.tcp_ref is not None and len(systemstat_global.tcp_ref) >= 6 and
        systemstat_global.jnt_ref is not None and len(systemstat_global.jnt_ref) >= 6
    ):
        current_tcp_ = Point(systemstat_global.tcp_ref[0], systemstat_global.tcp_ref[1],
                             systemstat_global.tcp_ref[2]
                             , systemstat_global.tcp_ref[3], systemstat_global.tcp_ref[4],
...
def ReqDataStart(sock):
    global cmd_connect
    global data_connect
    while cmd_connect and data_connect:
        try:
            msg = 'reqdata'
...
def ToCB(ip):
    if cmd_connect and data_connect:
        return True
    if cmd_connect or data_connect:
        DisConnectToCB()
        time.sleep(0.1)
    return ConnectToCB(ip)
